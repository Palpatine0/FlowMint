        ...
        <div className="max-w-6xl mx-auto flex flex-col gap-6">
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4 bg-white dark:bg-slate-800 p-1 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
                    <button onClick={handlePrevMonth} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg text-slate-600 dark:text-slate-300">
                        <ChevronLeft size={20} />
                    </button>
                    <span className="font-semibold px-4 min-w-[160px] text-center text-slate-800 dark:text-slate-200">
                        {currentDate.toLocaleDateString('default', { month: 'long', year: 'numeric' })}
                    </span>
                    <button onClick={handleNextMonth} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg text-slate-600 dark:text-slate-300">
                        <ChevronRight size={20} />
                    </button>
                    <button onClick={handleToday} className="px-4 py-1.5 text-sm font-medium text-primary-600 dark:text-primary-400 hover:bg-primary-50 dark:hover:bg-slate-700 rounded-lg mx-1 transition-colors">
                        Today
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                ...
                <div className="grid grid-cols-7 auto-rows-fr">
                    {totalCells.map((dayNumber, i) => {
                        ...
                        if (dayNumber === null) {
                            return <div key={`blank-${i}`} className="min-h-[140px] bg-slate-50/30 dark:bg-slate-800/30 border-b border-r border-slate-100 dark:border-slate-700" />;
                        }
                        ...
                        const isToday = new Date().toLocaleDateString() === new Date(year, month, dayNumber).toLocaleDateString();
                        ...
                        const income = dailyTxs.filter(t => t.type === 'income').reduce((s, t) => s + t.amount, 0);
                        const expense = dailyTxs.filter(t => t.type === 'expense').reduce((s, t) => s + t.amount, 0);
                        ...
                        return (
                            ...
                            >
                                <div className="flex justify-between items-start mb-2">
                                    <span className={`text-sm font-semibold w-7 h-7 flex items-center justify-center rounded-full ${isToday ? 'bg-primary-500 text-white shadow-md' : 'text-slate-700 dark:text-slate-300'}`}>
                                        {dayNumber}
                                    </span>
                                    {dailyTxs.length > 0 && (
