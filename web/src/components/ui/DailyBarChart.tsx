...
export function DailyBarChart({ transactions, activeRange }: Props) {
    ...
    if (activeRange === "week") {
        ...
        data = Array.from({ length: 12 }).map((_, i) => {
            const monthTxs = transactions.filter(t => new Date(t.date).getMonth() === i && new Date(t.date).getFullYear() === now.getFullYear());
        ...
        data = Array.from({ length: daysInMonth }).map((_, i) => {
            const day = i + 1;
            const dayTxs = transactions.filter(t => new Date(t.date).getDate() === day && new Date(t.date).getMonth() === now.getMonth());
            return {
                name: `${day}`,
                income: dayTxs.filter(t => t.type === "income").reduce((s, t) => s + t.amount, 0),
                expenses: dayTxs.filter(t => t.type === "expense").reduce((s, t) => s + t.amount, 0),
        ...
        const uniqueDates = Array.from(new Set(transactions.map(t => new Date(t.date).toLocaleDateString()))).sort((a, b) => new Date(a).getTime() - new Date(b).getTime());
        if (uniqueDates.length === 0) {
            data = Array.from({ length: 7 }).map((_, i) => {
                const d = new Date();
                d.setDate(d.getDate() - (6 - i));
                return { name: d.toLocaleDateString("en-US", { weekday: "short", day: "numeric" }), income: 0, expenses: 0 };
            });
        } else {
            data = uniqueDates.map(dateStr => {
                const dayTxs = transactions.filter(t => new Date(t.date).toLocaleDateString() === dateStr);
